...
public class DisplayContacts extends AppCompatActivity implements ScrollView.OnClickListener{
    ...
    private void makeDisplay() {
        ...
        if(people.size() > 0) {
            ...
            for (int i = 1; i < people.size(); i++) {
                ...
                //Log.d("Testing", "The new id is = " + id );
                screen.addView(currentLayout);

                Log.d("Testing", "The current layout id is " + currentLayout.getId());

            }
            id = currentLayout.getId();
            layout.add(id);
            sv.addView(screen);
            Log.d("Testing", "We just displayed all the contacts " + sv.getId());

            Log.d("Testing", "This is the find view by id" + findViewById(sv.getId()));
        }
    }
    ...
    public void onClick(View view) {
        ...
        if((boolean)getIntent().getExtras().get("VIEW")) {
            Log.d("Testing", "We just listened a click on id :" + (layout.indexOf(view.getId())));
            ...
            builder.setTitle("Delete " + people.get(x).getFname() + " " + people.get(x).getLname() + " from Contacts")
                    ...
                    .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialogInterface, int i) {
                            Log.d("Delete", "Successfully Removed the person from contacts list");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == EDIT_CODE){
            if(resultCode == RESULT_OK){
                ...
                Log.d("Edit", "A New Editted Contact is recieved");
                Log.d("Edit", data.getExtras().get(ECONTACT).toString() + " THIS IS IT");
